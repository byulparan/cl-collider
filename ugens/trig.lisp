...
(defmethod fold (in &optional (lo 0.0) (hi 1.0))
  (when (and (numberp in) (not (every #'numberp (list lo hi))))
    (error "Fold's lo and hi inputs cannot be UGens when its in input is a number."))
  (flet ((fold* (in lo hi)
           (let* ((x (- in lo))
                  (range (- hi lo))
                  (range2 (+ range range))
             ...
             (+ c lo))))
    (if (every #'numberp (list in lo hi)) (fold* in lo hi)
        (let ((op (lambda (cls in lo hi)
                    (declare (ignore cls))
                    (cond ((eql (rate in) :control) (fold.kr in lo hi))
                          ((eql (rate in) :audio) (fold.ar in lo hi))))))
...
(defmethod clip (in &optional (lo 0.0) (hi 1.0))
  (when (and (numberp in) (not (every #'numberp (list lo hi))))
    (error "Clip's lo and hi inputs cannot be UGens when its in input is a number."))
...
(defmethod wrap (in &optional (lo 0.0) (hi 1.0))
  (when (and (numberp in) (not (every #'numberp (list lo hi))))
    (error "Wrap's lo and hi inputs cannot be UGens when its in input is a number."))
  (if (every #'numberp (list in lo hi)) (+ (mod~ (-~ in lo) (-~ hi lo)) lo)
      (let ((op (lambda (cls in lo hi)
                  (declare (ignore cls))
                  (ecase (rate in)
                    (:control (wrap.kr in lo hi))
                    (:audio (wrap.ar in lo hi))))))
        (multinew op nil in lo hi))))
