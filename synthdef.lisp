...
(defun make-control (params rate)
  ...
  (labels ((make-ctrl (params)
    ...
    (let* ((trig-controls (remove-if-not #!(eql :tr (third %)) params))
      ...
      (setf (control-names *synthdef*)
	    (loop for all-controls in (append trig-controls controls)
		  and index = 0 then (incf index (length (su:mklist (second all-controls))))
		  for containers = nil
      ...
      (append
       (when trig-controls
	 (ugen-new "TrigControl" rate 'control #'identity :bipolar (mapcar #'second trig-controls) 0))
       (when controls
	 (ugen-new (if lag-p "LagControl" "Control")
		   rate'control #'identity :bipolar (mapcar #'second controls)
		   (length (alexandria:flatten (mapcar #'second trig-controls)))
