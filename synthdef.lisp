    ...
    `(let* ((,node (gethash ,key (node-proxy-table *s*)))
       ...
       ,(if body
	    (alexandria:once-only (id fade)
	      `(labels ((clear-node ()
			  (when ,node-alive-p
			    (if ,has-fade (ctrl ,node :gate 0 :fade (* ,rel ,fade))
			      (free ,node)))))
...
(defun proxy-ctrl (key &rest params &key (rel 1) (fade .5) &allow-other-keys)
  (let* ((name key)
    ...
    (let* ((fade-time (synthdef-metadata key :fade-time)))
      (flet ((clear-node ()
	       (if fade-time (ctrl node :gate 0 :fade (* rel fade))
		   (free node))))
